...
* 本地全局变量
*********************************************************************************************/
static uint8 f_szGetdqy[8]={0x01,0x03,0x00,0x00,0x00,0x01,0x84,0x0A};              //获取氨气命令,采用RS485通讯方式，
                                                                //通讯协议采用ModBus通讯协议
static float dqy_VAL = 0.0;                                     //缓存氨气值
...
/*********************************************************************************************
* 名称：phs_update()
* 功能：更新一次光照度值，保存到全局静态变量(dqy_VAL)中
* 参数：无
* 返回：无
* 修改：
* 注释：
...
{ 
  uart_485_write(f_szGetdqy, sizeof(f_szGetdqy));
}
    ...
    if (len > 0){                                               //如果长度大于0
      ...
      if (len == 7){                                            //如果长度为7，数据接收完毕
        ...
        if (crc == ((szBuf[6]<<8) | szBuf[5])){                 //如果正确
          dqy_VAL = (float)((szBuf[3]<<8) | szBuf[4]);          //提取传感器采集光照强度值
        }
        len = 0;                                                //数据提取成功，len置零
      }
    }else{
