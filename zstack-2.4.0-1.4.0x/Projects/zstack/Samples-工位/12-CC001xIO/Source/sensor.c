...
//定义窗磁传感器信号检测接口
#define D_WIN_SEL                  P0SEL                        
#define D_WIN_DIR                  P0DIR

#define D_WIN_SBIT                 P0_4
#define D_WIN_BV                   BV(4)
/*********************************************************************************************
* 全局变量
*********************************************************************************************/
static uint8 D0 = 3;                                            // 默认打开主动上报功能
static uint8 D1 = 0;                                            // 继电器初始状态为全关
static uint8 A0 = 0;                                            // A0存储窗磁报警值
static uint16 V0 = 60;                                          // V0设置为上报时间间隔，默认为30s
/*********************************************************************************************
* 名称：updateV0()
* 功能：更新V0的值
* 参数：*val -- 待更新的变量
...
{
  //配置窗磁传感器信号检测接口
  D_WIN_SEL &= ~D_WIN_BV;                                       // 配置传感器信号接收端口
  D_WIN_DIR &= ~D_WIN_BV;                                       // 配置接口为输入方向
...
{ 
  ...
  // 根据D0的位状态判定需要主动上报的数值
  if ((D0 & 0x01) == 0x01){                                     // 若窗磁报警值上报允许，则pData的数据包中添加窗磁报警值数据
    updateA0();
    sprintf(p, "%u", A0); 
    ZXBeeAdd("A0", p);
  }
...
void sensorCheck(void)
{
  ...
  updateA0();
  if(last_A0 != A0)                                             // 如果状态出现改变
  {
    sensorUpdate();                                             // 上报当前窗磁状态
    last_A0 = A0;
  }
}
/*********************************************************************************************
