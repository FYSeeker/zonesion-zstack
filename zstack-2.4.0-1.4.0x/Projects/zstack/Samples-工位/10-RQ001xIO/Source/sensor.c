...
* 作者：Zhoucj 2018.6.25
* 说明：燃气传感器
*       节点具备二个属性：实时监测燃气传感器状态报警；远程控制燃气传感器开关
*       变量A0表示燃气报警状态
*       V0表示燃气报警主动上报时间间隔
*       D0(Bit0)分别表示A0主动上报使能
*       D1表示燃气传感器继电器开关状态，OD1/CD1进行控制
*       默认值：A0=0,V0=30,D0=3,D1=1
* 修改：
* 注释：
*********************************************************************************************/
...
#define D_GAS_BET               P0_4                            // 宏定义燃气检测引脚
/*********************************************************************************************
* 全局变量
*********************************************************************************************/
static uint8 D0 = 3;                                            // 默认打开主动上报功能
static uint8 D1 = 1;                                            // 继电器初始状态为打开继电器1
static uint8 A0 = 0;                                            // A0存储燃气报警值
static uint16 V0 = 60;                                          // V0设置为上报时间间隔，默认为60s
/*********************************************************************************************
* 名称：updateV0()
* 功能：更新V0的值
* 参数：*val -- 待更新的变量
...
void updateA0(void)
{
    if (D_GAS_BET)                                                // 判断管脚的电平状态值
    {
        A0 = 1;                                                     // 检测为高电平时没有检测到燃气A0=1
    }
    else
    {
        A0 = 0;                                                     // 否则检测到燃气A0=0
...
void sensorUpdate(void)
{
    ...
    // 根据D0的位状态判定需要主动上报的数值
    if ((D0 & 0x01) == 0x01)                                      // 若燃气报警值上报允许，则pData的数据包中添加燃气报警值数据
    {
        updateA0();
        sprintf(p, "%u", A0);
        ZXBeeAdd("A0", p);
