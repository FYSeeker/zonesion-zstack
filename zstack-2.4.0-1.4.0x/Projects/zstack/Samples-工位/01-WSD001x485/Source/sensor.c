...
{ 
  ...
  if ((D0 & 0x02) == 0x02){                                     // //若检测湿度值允许上报，则pData的数据包中添加湿度数据
    A1 = updateA1();                                          //更新湿度值
    sprintf(p, "%.1f", A1); 
    ZXBeeAdd("A1", p);
  }
...
void MyEventProcess( uint16 event )
{
  ...
  if (event & MY_CHECK_EVT) {
    hts_update();                                             //每100毫秒获取一次温湿度值
    //sensorCheck(); 
    // 启动定时器，触发事件：MY_CHECK_EVT 
    osal_start_timerEx(sapi_TaskID, MY_CHECK_EVT, 3000);
  } 
}