...
* 全局变量
*********************************************************************************************/
static uint8 f_szGetTempHumi[8]={0x01,0x03,0x00,0x00,0x00,0x03,0x05,0xCB};//获取温湿度命令
...
#define RBUFSIZE 128
  ...
  while (Hal_UART_RxBufLen(port))
  {
    ...
    if (len > 0) 
    {
      ...
      {
        uint16 crc;
        crc = calc_crc(szBuf, 9);
        if (crc == ((szBuf[10]<<8) | szBuf[9])) 
        {
          int nTemp, nHumi;
          nHumi = (szBuf[3]<<8) | szBuf[4];
          nTemp = (szBuf[5]<<8) | szBuf[6];
          
          f_fHumi = (float)nHumi/100.0f;
          f_fTemp = (float)(nTemp - 27315)/100.0f;
        }
        len = 0;
      }
    } 
