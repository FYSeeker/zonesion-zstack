...
* 本地全局变量
*********************************************************************************************/
static uint8 f_szGetNoise[8]={0x01,0x03,0x00,0x00,0x00,0x01,0x84,0x0A};//获取噪声命令,采用RS485通讯方式，
                                                                
static float Noise_VAL = 0.0;                                   //缓存噪声值
...
/*********************************************************************************************
* 名称：phs_update()
* 功能：更新一次光照度值，保存到全局静态变量(Noise_VAL)中
* 参数：无
* 返回：无
* 修改：
* 注释：
...
{ 
  uart_485_write(f_szGetNoise, sizeof(f_szGetNoise));
}
...
static void uart_callback_func(uint8 port, uint8 event) 
{
  if (event & HAL_UART_TX_EMPTY){
    uint16 szDelay[] = {2000,1000,500,100,10,10000,4000};
    ...
    if (len > 0){                                               //如果长度大于0
      ...
      if (len == 7){                                            //如果长度为7，数据接收完毕
        ...
        if (crc == ((szBuf[6]<<8) | szBuf[5])){                 //如果正确
          Noise_VAL = (szBuf[3]<<8) | szBuf[4];          //提取传感器采集噪音值
        }
        len = 0;                                                //数据提取成功，len置零
      }
    }else{
