...
* 全局变量
*********************************************************************************************/
static uint8  switchStatus = 0;                                           // 继电器初始状态为全关
/*********************************************************************************************
* 名称：sensorInit()
* 功能：传感器硬件初始化
* 参数：无
...
{ 
  ...
  sprintf(p, "%u", switchStatus);                                  // 上报控制编码 
  ZXBeeAdd("switchStatus", p);
...
{ 
  ...
  if (0 == strcmp("switchStatus", ptag)){                        // 查询执行器命令编码
    if (0 == strcmp("?", pval)){
      ret = sprintf(p, "%u", switchStatus);
      ZXBeeAdd("switchStatus", p);
    } 
  }
