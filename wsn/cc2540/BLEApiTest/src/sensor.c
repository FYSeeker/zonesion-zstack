...
* 全局变量
*********************************************************************************************/
static float humidity = 0.0;                              // 光照强度值
/*********************************************************************************************
* 名称：sensorInit()
* 功能：传感器硬件初始化
* 参数：无
...
{ 
  ...
  // 湿度度采集集（0~100随机数）
  humidity = (uint16)(osal_rand()%100);
  
  // 更新湿度的值
  sprintf(p, "humidity=%.1f", humidity);
  bleSend(p, strlen(p));
...
void sensorControl(uint8 cmd)
{
  ...
  if(cmd == 1){ 
    RELAY = ON;                                                 // 开启LED
    printf("sensor->sensorControl(): LED ON\r\n");
  }
  else if(cmd == 0){
    RELAY = OFF;                                                // 关闭LED
    printf("sensor->sensorControl(): LED OFF\r\n");
  }
}
/*********************************************************************************************
* 名称：ZXBeeInfRecv()
...
void ZXBeeInfRecv(char *pkg, int len)
{
  ...
  printf("sensor->ZXBeeInfRecv(): Receive BLE Data!\r\n");
  HalLedSet(HAL_LED_1, HAL_LED_MODE_BLINK);
