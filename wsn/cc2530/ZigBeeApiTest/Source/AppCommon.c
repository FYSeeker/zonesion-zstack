...
void zb_HandleOsalEvent( uint16 event )
{
  if (event & ZB_ENTRY_EVENT) {
    ...
    at_init();
    printf("AppCommon->zb_HandleOsalEvent(): ZB_ENTRY_EVENT trigger!\r\n");
  ...
  if (event & 0x000F) {                                         // 触发用户自定义事件
    printf("AppCommon->zb_HandleOsalEvent(): MyEvent trigger!\r\n");
...
void zb_StartConfirm( uint8 status )
{
  ...
  {
    printf("AppCommon->zb_StartConfirm(): Join ZigBee Net Success!\r\n");
...
void zb_ReceiveDataIndication( uint16 source, uint16 command, uint16 len, uint8 *pData  )
{
  ...
  printf("AppCommon->zb_ReceiveDataIndication(): Receive ZigBee Data!\r\n");  
