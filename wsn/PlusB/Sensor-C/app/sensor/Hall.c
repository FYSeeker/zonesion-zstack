...
void hall_init(void)
{
  ...
  RCC_AHB1PeriphClockCmd(HALL_RCC, ENABLE);                     //开启霍尔相关的GPIO外设时钟
  
  GPIO_InitStructure.GPIO_Pin = HALL_PIN;                       //选择要控制的GPIO引脚
  ...
  GPIO_Init(HALL_GPIO, &GPIO_InitStructure);                    //初始化GPIO配置
}
...
{ 
  if(GPIO_ReadInputDataBit(HALL_GPIO,HALL_PIN))
    return 0;
  else
    return 1;
}